...
class Poisson2D:
    ...
    def laplace(self):
        D2x = self.D2()
        D2y = self.D2()
        return sparse.kron(D2x, sparse.eye(self.N + 1)) + sparse.kron(
    ...
    def l2_error(self, u):
        ...
        return np.sqrt(self.h ** 2 * np.sum((u - ue_exact) ** 2))
    ...
    def eval(self, x, y):
        ...
        points = (self.xij[:, 0], self.yij[0, :])
        interp_points = np.array([x, y])
        u_interpn = interpn(points, self.U, interp_points, method="linear")
        return float(u_interpn)
