...
class Wave2D:
    ...
    def initialize(self, N, mx, my):
        ...
        self.Unm1[:] = sp.lambdify((x, y, t), self.ue(self.mx, self.my))(
            self.xij, self.yij, 0
        )

        self.Un[:] = self.Unm1[:] + 0.5 * (self.c * self.dt) ** 2 * (
            D2 @ self.Unm1[:] + self.Unm1[:] @ D2.T
        )
        self.Un[0] = 0
        self.Un[-1] = 0
        self.Un[:, 0] = 0
    ...
    def __call__(self, N, Nt, cfl=0.5, c=1.0, mx=3, my=3, store_data=-1):
        ...
        for n in range(2, Nt + 1):
            self.Unp1[:] = (
                2 * self.Un
                - self.Unm1
                + (self.c * dt) ** 2 * (D2 @ self.Un + self.Un @ D2.T)
            )
            self.apply_bcs()
...
class Wave2D_Neumann(Wave2D):
    ...
    def initialize(self, N, mx, my):
        ...
        self.Unm1[:] = sp.lambdify((x, y), self.ue(mx, my).subs(t, 0))(
            self.xij, self.yij
        )
        self.Un[:] = self.Unm1[:] + 0.5 * (self.c * self.dt) ** 2 * (
            D2 @ self.Unm1 + self.Unm1 @ D2.T
